...
git checkout -b $TEST && npm version $TEST -m "increase version [ci skip]" && git push -f origin $TEST && git push -f --tags && git checkout master && npm version $(semver $(semver $ACTUAL_VERSION -i minor) -i minor)-dev.0 -m "increase version [ci skip]" && git push -f origin $BRANCH && git push -f --tags && $POST_RUN_COMMAND
